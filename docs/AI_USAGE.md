...
- Thinking default: disabled (`think: false` for supported qwen3 models)
...
The AI assistant uses RAG (retrieval augmented generation). It performs evidence retrieval from local data, lets the user select evidence, and sends only the selected evidence to the local LLM prompt.
